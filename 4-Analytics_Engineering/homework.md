...
```sql
select *
FROM adaora.dbt_aasika.fct_taxi_trips_quarterly_revenue
WHERE year = 2020
ORDER BY service_type, yoy_rev_growth_percentage DESC;
```
...
```sql
SELECT distinct * 
FROM `adaora.dbt_aasika.fct_taxi_trips_monthly_fare_p95`
...
WITH ranked_trips AS (
    SELECT *,
        DENSE_RANK() OVER (PARTITION BY pickup_zone ORDER BY p90_trip_duration DESC) AS trip_rank
    FROM `adaora.dbt_aasika.fct_fhv_monthly_zone_traveltime_p90`
